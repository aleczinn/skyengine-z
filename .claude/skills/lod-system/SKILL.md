...
Shader, gleiche OPAQUE-Arena, gleiche Indirect-Draws wie echtes Terrain.
...
- `ChunkRenderer` übernimmt Ergebnisse (Budget 4/Frame), hängt sichtbare LOD-Draws ans
  OPAQUE-Segment und räumt Meshes ab, sobald `isDesiredKey` false ist.
...
3. **16-Bit-Chunk-Maske:** Bit gesetzt = dieser Chunk zeigt JETZT echtes Terrain — d.h. Status
   READY **und** `chunk.isFullyUploaded()` (alle 16 Section-Uploads vom Renderer angewendet).
   READY allein reicht nicht → Löcher flackern, bevor der echte Mesh sichtbar ist. Masken-Diff
   (Chunk fertig geladen/entladen) triggert Region-Remesh. Innen wird NICHT per Radius
   ausgeschlossen — die Maske clippt zellgenau und füllt Lücken, solange Chunks laden.
